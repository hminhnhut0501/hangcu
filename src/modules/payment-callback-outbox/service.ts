...
export async function retryPaymentCallbackOutbox(limit = 10) {
  ...
  const callbackUrl = process.env.LICENSE_BOT_PAYMENT_STATUS_URL?.trim() || process.env.BOT_PAYMENT_STATUS_URL?.trim();
  ...
  for (const row of data ?? []) {
    ...
    try {
      const response = await fetch(callbackUrl.replace(/\/$/, ""), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(row.payload) });
